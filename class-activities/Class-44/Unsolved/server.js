...
app.get("/markread/:id", function(req, res) {
  ...
  db.books.update({_id:mongojs.ObjectId(req.params.id)},{$set:{read:true}},(err,updated) => {
    if(err){
      console.log(err);
    }
    else{
...
app.get("/markunread/:id", function(req, res) {
  // Remember: when searching by an id, the id needs to be passed in
  // as (mongojs.ObjectId(IdYouWantToFind))
  db.books.update({_id:mongojs.ObjectId(req.params.id)},{$set:{read:false}},(err,updated) => {
    if(err){
      console.log(err);
    }
    else{
